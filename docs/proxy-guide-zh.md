...
YKW Bridge 的代理功能是一个本地 HTTP 代理服务器，可以统一管理 Claude Code、Codex 和 Gemini CLI 的 API 请求。主要特性包括：
...
> **注意**：启用接管后，YKW Bridge 会自动修改对应应用的配置文件，将 API 端点指向本地代理。原始配置会被安全备份。

### 3. 正常使用 CLI

启用接管后，你可以正常使用各个 CLI 工具。所有请求都会经过 YKW Bridge 代理转发到配置的供应商。
...
这确保了即使 YKW Bridge 异常退出，重新启动后也能正确识别状态。
...
A: 点击 **停止代理** 按钮，YKW Bridge 会自动恢复所有应用的原始配置。
