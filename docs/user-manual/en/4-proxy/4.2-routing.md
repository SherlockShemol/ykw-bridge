...
App routing means letting YKW Bridge route a specific application's API requests through the local routing service.
...
When routing is enabled, YKW Bridge modifies the app's configuration file to point the API endpoint to the local routing service.
...
When disabling routing, YKW Bridge will:
...
Before enabling routing, YKW Bridge backs up the original configuration and restores it when disabled.
