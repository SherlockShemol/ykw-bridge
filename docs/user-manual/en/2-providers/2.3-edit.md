...
YKW Bridge provides rich icon customization features:
...
This ensures YKW Bridge and CLI tool configurations stay in sync.
...
When adding a provider, YKW Bridge automatically extracts endpoint URLs from the configuration.
...
3. Claude Code usually picks up the change immediately; if not, restart the CLI tool once
