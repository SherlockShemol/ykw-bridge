...
YKW Bridge supports the `ccswitch://` deep link protocol, enabling one-click configuration import via links.
...
YKW Bridge provides an online deep link generator tool:
...
Using YKW Bridge's official online deep link generator tool is more convenient.
...
1. The system asks whether to open YKW Bridge
2. After confirming, YKW Bridge opens
...
YKW Bridge automatically registers the `ccswitch://` protocol during installation.
...
```bash
/usr/bin/open -a "YKW Bridge" --args --register-protocol
```
