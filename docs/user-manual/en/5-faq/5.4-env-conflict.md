...
YKW Bridge automatically detects conflicts between system environment variables and app configurations, preventing configurations from being unexpectedly overridden.
...
Found X environment variables that may conflict with YKW Bridge configuration
[Expand] [Dismiss]
```
...
3. YKW Bridge will automatically back up and remove the selected variables
...
If you prefer not to use YKW Bridge to remove variables, you can handle them manually:
...
- YKW Bridge provider configurations being overridden
- API requests being sent to the wrong endpoint
- Using the wrong API key

## Best Practices

1. **Use YKW Bridge to manage configurations**: Avoid setting API keys in system environment variables
