...
YKW Bridge for macOS is code-signed and notarized by Apple. You can download and install it directly without any additional steps. If you encounter issues, try downloading the latest version from the [Releases page](https://github.com/SherlockShemol/ykw-bridge/releases).
...
2. Add YKW Bridge to your antivirus software's whitelist
...
**Windows**:
- Check taskbar settings to ensure the YKW Bridge icon is not hidden
...
Toggle "Lightweight Mode" from the system tray menu. The main window closes, and YKW Bridge runs as a tray-only app. Toggle again or click "Open main window" to exit.
...
Yes. Starting from v3.13.0, YKW Bridge covers all window re-show paths (normal launch, deep links, singleton activation, tray `show_main`, and Lightweight Mode return). Clicking a `ccswitch://` link **rebuilds the main window on demand** and displays the import confirmation dialog. The first open is slightly slower than normal state (window rebuild required), but subsequent switches return to normal speed.
