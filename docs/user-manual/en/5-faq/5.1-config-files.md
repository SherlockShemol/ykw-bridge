...
| Field | Description |
|-------|-------------|
| `models.providers` | Provider configuration (mapped to YKW Bridge's "providers") |
...
YKW Bridge's priority when modifying configurations:

1. **YKW Bridge Database** - Single source of truth (SSOT)
...
- CLI tool configuration files (will be backfilled by YKW Bridge)
- YKW Bridge's `settings.json`
...
YKW Bridge v3.7.0 migrated from JSON files to SQLite:
