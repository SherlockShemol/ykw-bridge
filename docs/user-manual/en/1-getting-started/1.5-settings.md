...
This section describes how to configure YKW Bridge according to your preferences.
...
YKW Bridge supports three languages:
...
When enabled, YKW Bridge automatically runs when the system starts.
...
Starting from v3.13.0, YKW Bridge adds **Lightweight Mode** — a **tray-only** running state that minimizes desktop footprint when idle.
...
**How to exit**: Click **Open Main Window** from the tray menu, or trigger YKW Bridge via deep link / relaunch. The window is **rebuilt on demand**, with state preserved.
...
> **Use case**: If YKW Bridge runs in the background for long periods and you mainly switch providers via the tray menu, enabling Lightweight Mode significantly reduces memory usage.
...
When enabled, YKW Bridge automatically syncs the configuration to the VS Code Claude Code extension (writes `primaryApiKey` to `~/.claude/config.json`) when switching providers.
...
This Claude-focused edition keeps navigation centered on Claude workflows. If a migrated profile still shows legacy entries, hide them here to keep the interface clean.
...
Choose the terminal application that YKW Bridge uses when opening a terminal.
...
The storage location for YKW Bridge's own data, defaulting to `~/.ykw-bridge/`.

### CLI Tool Directories

You can customize the Claude Code configuration directory:

| Setting | Default | Description |
|---------|---------|-------------|
| Claude Directory | `~/.claude/` | Claude Code configuration directory |

> **Note**: After changing directories, restart the app and make sure Claude Code uses the same directory. Claude Desktop app path and profile settings are configured in the dedicated Claude Desktop section.
...
Configure failover queues and automatic switching strategies for Claude provider chains. See [4.3 Failover](../4-proxy/4.3-failover.md) for details.
...
Configure YKW Bridge's outbound HTTP/HTTPS proxy, applicable for scenarios where external API access requires a proxy.
...
When an interval is set, YKW Bridge automatically backs up the database on schedule. Older backups beyond the retention count are automatically removed.
...
| Remote Directory | Remote storage path (default: `ykw-bridge-sync`) |
...
- YKW Bridge automatically syncs the database to WebDAV at the configured interval
- Sync status is displayed in the panel

#### Remote Snapshot Info

Before downloading, YKW Bridge displays details about the remote snapshot:
- Protocol version (v2)
...
Displays the current YKW Bridge version number, with support for:
...
Automatically detects the installed Claude Code version:
...
Provides a quick command to install/update Claude Code:
