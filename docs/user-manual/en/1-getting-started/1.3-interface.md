...
| 4 | App Switcher | Stay in the Claude workspace and manage Claude providers |
| 5 | Feature Area | Skills / Prompts / MCP entry points |
| 6 | Add Button | Add a new provider |

### App Switcher

Use the dropdown menu to enter the currently managed Claude workspace:

- **Claude** - Manage Claude Code and Claude Desktop configuration

The provider list below displays the currently available Claude providers.
...
YKW Bridge displays an icon in the system tray, providing quick access to operations.
...
| Claude Submenu | Collapsible submenu for Claude providers (e.g., "Claude · PackyCode") |
...
> **Note**: The Claude submenu title shows the current provider name (e.g., "Claude · PackyCode"). If no provider is configured, it shows a disabled "(no providers)" entry. Visibility is controlled by the App Visibility setting.
