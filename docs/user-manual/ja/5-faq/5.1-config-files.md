...
| フィールド | 説明 |
|------|------|
| `models.providers` | プロバイダー設定（YKW Bridge の「プロバイダー」にマッピング） |
| `env` | 環境変数設定 |
...
YKW Bridge が設定を変更する際の優先順位：

1. **YKW Bridge データベース** - 単一事実源 (SSOT)
...
- CLI ツールの設定ファイル（YKW Bridge がバックフィルする）
...
YKW Bridge v3.7.0 で JSON ファイルから SQLite に移行しました：
