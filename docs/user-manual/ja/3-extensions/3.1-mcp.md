...
あるアプリのスイッチをオンにすると、YKW Bridge は以下を実行します：
...
あるアプリのスイッチをオフにすると、YKW Bridge は以下を実行します：
...
CLI ツールで既に MCP サーバーを設定している場合、YKW Bridge にインポートできます：

1. 「インポート」ボタンをクリック
2. インポートするアプリを選択（Claude/Codex/Gemini/OpenCode）
3. YKW Bridge が既存の設定を読み取ってインポート
