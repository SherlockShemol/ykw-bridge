...
YKW Bridge v3.9.0 は v3.9 ベータ（`3.9.0-1`、`3.9.0-2`、`3.9.0-3`）の安定版です。
...
- SQL インポートは YKW Bridge がエクスポートしたバックアップのみに制限されます（安全性のため）。
- プロキシのテイクオーバーは CLI の live 設定を変更します。YKW Bridge はリダイレクト前に live 設定をバックアップします。元に戻す場合はテイクオーバー無効化/プロキシ停止を行い、必要に応じてバックアップから復元してください。
...
[Releases](https://github.com/SherlockShemol/ykw-bridge/releases/latest) から該当するバージョンをダウンロードしてください。
...
| ファイル                                | 説明                                         |
| --------------------------------------- | -------------------------------------------- |
| `YKW-Bridge-v3.9.0-Windows.msi`          | **推奨** - MSI インストーラー、自動更新対応  |
| `YKW-Bridge-v3.9.0-Windows-Portable.zip` | ポータブル版、インストール不要               |

### macOS

| ファイル                        | 説明                                                              |
| ------------------------------- | ----------------------------------------------------------------- |
| `YKW-Bridge-v3.9.0-macOS.zip`    | **推奨** - 解凍して Applications へドラッグ、Universal Binary     |
| `YKW-Bridge-v3.9.0-macOS.tar.gz` | Homebrew インストールおよび自動更新用                             |
...
| --------------------------------------- | ----------- | ------------------------------------------------------------------------------ |
| Ubuntu / Debian / Linux Mint / Pop!\_OS | `.deb`      | `sudo dpkg -i YKW-Bridge-*.deb` または `sudo apt install ./YKW-Bridge-*.deb`     |
| Fedora / RHEL / CentOS / Rocky Linux    | `.rpm`      | `sudo rpm -i YKW-Bridge-*.rpm` または `sudo dnf install ./YKW-Bridge-*.rpm`      |
| openSUSE                                | `.rpm`      | `sudo zypper install ./YKW-Bridge-*.rpm`                                        |
| Arch Linux / Manjaro                    | `.AppImage` | 実行権限を付与して直接実行、または AUR を使用                                  |
| その他 / 不明                           | `.AppImage` | `chmod +x YKW-Bridge-*.AppImage && ./YKW-Bridge-*.AppImage`                      |
| サンドボックスで実行したい場合          | `.flatpak`  | `flatpak install YKW-Bridge-*.flatpak`                                          |
