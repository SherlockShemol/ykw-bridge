...
### Claude Code + Claude Desktop 専用マネージャー
...
Claude ワークフローでは、provider 設定、プロキシ、MCP、Prompts、Skills に加えて、Claude Desktop 独自の証明書や起動チェーンまで面倒を見る必要があります。JSON を手で編集し続ける運用は壊れやすいままです。

**YKW Bridge** は現在、**Claude Code** と **Claude Desktop** に集中したアプリです。Claude 側の provider 管理、managed auth、MCP、Prompts、Skills、Sessions、Usage、そして Claude Desktop の doctor / certificate / launch 周りを 1 か所で扱えます。

- **Claude 専用の 1 アプリ** -- Claude Code と Claude Desktop を単一インターフェースで管理
- **手動編集は不要** -- provider を可視化して素早く切り替えられる
- **統一 MCP・Skills 管理** -- Claude 側の設定だけに集中できる
...
- **Claude provider 管理** -- インポート、切り替え、並び替え、エクスポートを一か所で
- **managed auth を維持** -- GitHub Copilot OAuth と ChatGPT / OpenAI OAuth を引き続き利用可能
...
- **アプリレベルのテイクオーバー** -- Claude Code と Claude Desktop を個別に制御

### MCP、Prompts & Skills

- **統一 MCP パネル** -- Claude MCP を管理、Deep Link インポート対応
- **Prompts** -- Claude 向け Markdown プロンプト編集とバックフィル保護
...
- Claude の会話履歴を閲覧・検索・復元
- **Claude Desktop ツール** -- doctor、証明書、launch shim、watchdog、takeover を一元管理
...
<details>
<summary><strong>YKW Bridge は現在どのツールに対応していますか？</strong></summary>

この専用版は **Claude Code** と **Claude Desktop** を対象にしています。`GitHub Copilot` と `ChatGPT / OpenAI` の認証は、Claude 向け provider 管理の一部として引き続き利用できます。
...
YKW Bridge は「最小限の介入」という設計原則に従っています。アプリをアンインストールしても、Claude Code は正常に動作し続けます。すべての設定を削除すると Claude が使えるプロバイダを失うため、システムは常にアクティブな設定を 1 つ保持します。公式ログインに戻す方法は、次の質問をご覧ください。
...
プリセットリストから公式 Claude プロバイダを追加してください。切り替え後、ログアウト／ログインのフローを実行すれば、以降は公式プロバイダとサードパーティプロバイダを自由に切り替えられます。
