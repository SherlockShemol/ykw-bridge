...
pub async fn check_for_updates(handle: AppHandle) -> Result<bool, String> {
    handle
        .opener()
        .open_url(
            "https://github.com/SherlockShemol/ykw-bridge/releases/latest",
            None::<String>,
        )
...
mod tests {
    ...
    #[test]
    fn parse_terminal_provider_app_rejects_unsupported_non_claude_apps() {
        let err = parse_terminal_provider_app("legacy_non_claude")
    ...
    fn resolve_launch_cwd_rejects_missing_directory() {
        let unique = std::time::SystemTime::now()
            ...
            .as_nanos();
        let missing = std::env::temp_dir().join(format!("ykw-bridge-missing-{unique}"));
