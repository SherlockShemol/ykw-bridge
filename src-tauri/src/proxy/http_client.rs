...
pub fn set_proxy_port(port: u16) {
    if let Some(lock) = CC_SWITCH_PROXY_PORT.get() {
        if let Ok(mut current_port) = lock.write() {
            *current_port = port;
            log::debug!("[GlobalProxy] Updated YKW Bridge proxy port to {port}");
        }
    } else {
        let _ = CC_SWITCH_PROXY_PORT.set(RwLock::new(port));
        log::debug!("[GlobalProxy] Initialized YKW Bridge proxy port to {port}");
    }
}

/// 获取 YKW Bridge 代理服务器的监听端口
...
mod tests {
    ...
    fn test_proxy_points_to_loopback() {
        // 设置 YKW Bridge 代理端口为 15721（默认值）
        set_proxy_port(15721);

        // 只有指向 YKW Bridge 自己端口的 loopback 地址才返回 true
