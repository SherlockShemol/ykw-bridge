...
pub fn anthropic_to_responses(
    ...
    //   （ykw-bridge 的 transform 当前是"条件写入"，可能产生缺失）
    // - stream: 必须永远 true（codex-rs 硬编码 true，且 ykw-bridge 的
...
pub(crate) fn map_responses_stop_reason(
    ...
    status.map(|s| match s {
        ...
        "incomplete"
            if matches!(
                incomplete_reason,
                None | Some("max_output_tokens") | Some("max_tokens")
            ) =>
        {
            "max_tokens"
        }
