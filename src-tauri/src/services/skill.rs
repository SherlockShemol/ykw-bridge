...
pub enum SkillStorageLocation {
    /// YKW Bridge 管理目录 (~/.ykw-bridge/skills/)
    #[default]
    CcSwitch,
...
impl SkillService {
    ...
    /// 获取 SSOT 目录（根据设置返回 ~/.ykw-bridge/skills/ 或 ~/.agents/skills/）
    ...
    /// 获取 Skill 卸载备份目录（~/.ykw-bridge/skill-backups/）
    ...
    pub fn get_app_skills_dir(app: &AppType) -> Result<PathBuf> {
        ...
        match app {
            ...
            AppType::ClaudeDesktop => {
                if let Some(custom) = crate::settings::get_claude_desktop_profile_dir() {
                    return Ok(custom.join("skills"));
                }
            }
        }
    ...
    /// 扫描未管理的 Skills
    ///
    /// 扫描各应用目录，找出未被 YKW Bridge 管理的 Skills
    pub fn scan_unmanaged(db: &Arc<Database>) -> Result<Vec<UnmanagedSkill>> {
        ...
        if let Ok(ssot_dir) = Self::get_ssot_dir() {
            scan_sources.push((ssot_dir, "ykw-bridge".to_string()));
        }
    ...
    /// 从应用目录导入 Skills
    ///
    /// 将未管理的 Skills 导入到 YKW Bridge 统一管理
    pub fn import_from_apps(
        ...
        search_sources.push((ssot_dir.clone(), "ykw-bridge".to_string()));
